...
const FeedbackForm = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Reset form
    const updatedRatings = ratingCategories.map(cat => ({...cat, value: 0}));
    setRatingCategories(updatedRatings);
    setFollowUp(false);
  ...
  const renderStars = (categoryIndex: number) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        ...
        >
          <Star className={i <= category.value ? "fill-levain-brown" : ""} size={24} />
        </button>
      );
    }
  ...
  return (
    <section id="feedback" className="cafe-section relative overflow-hidden w-full">
      ...
      <div className="absolute top-24 right-40 flex opacity-20">
        {[...Array(5)].map((_, i) => (
          <Star key={i} size={24} className={i < 3 ? "fill-levain-brown text-levain-brown" : "text-levain-brown"} />
        ))}
      </div>
      <div className="absolute bottom-24 left-40 flex opacity-20">
        {[...Array(5)].map((_, i) => (
          <Star key={i} size={20} className={i < 4 ? "fill-levain-brown text-levain-brown" : "text-levain-brown"} />
        ))}
      </div>
      ...
      <div className="absolute left-0 top-0 bottom-0 w-12 flex flex-col justify-around items-center opacity-5">
        {[...Array(10)].map((_, i) => (
          <Star 
            key={i} 
            size={i % 2 === 0 ? 24 : 16} 
            className={i % 3 === 0 ? "fill-levain-brown text-levain-brown" : "text-levain-brown"} 
          />
        ))}
      </div>
          ...
          <div key={i} className="flex flex-col items-center">
            ...
            {[...Array(5)].map((_, j) => (
              <Star 
                key={`${i}-${j}`} 
                size={12} 
                className={(j < i+1) ? "fill-levain-brown text-levain-brown" : "text-levain-brown"} 
              />
            ))}
          </div>
        ))}
      ...
      <div className="absolute top-1/3 left-0 right-0 w-screen h-8 flex justify-around items-center opacity-5">
        {[...Array(40)].map((_, i) => (
          <Star 
            key={i} 
            size={12} 
            className={(i % 5 < 4) ? "fill-levain-brown text-levain-brown" : "text-levain-brown"} 
          />
        ))}
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 w-screen h-6 bg-levain-brown/5 flex justify-around items-center">
        {[...Array(30)].map((_, i) => (
          <div key={i} className="w-3 h-3 bg-levain-brown/10 rounded-full"></div>
        ))}
      </div>
      ...
      <div className="absolute -left-6 top-1/4 transform -translate-y-1/2">
        <div className="w-12 h-12 rounded-full bg-levain-brown/5 flex items-center justify-center">
          <Star size={24} className="text-levain-brown/30 fill-levain-brown/30" />
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-6">
                ...
                {ratingCategories.map((category, index) => (
                  <div key={category.name} className="flex flex-col space-y-2 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                    <Label htmlFor={category.name} className="text-sm font-medium min-w-[140px]">
                      {category.label}:
                    </Label>
              ...
              <div className="grid gap-6 pt-4 border-t border-levain-brown/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <Input id="name" placeholder="Your name" className="mt-2 input-field" />
                  </div>
                  <div>
                    <Label htmlFor="phone">Phone Number:</Label>
                    <Input id="phone" type="tel" placeholder="Your phone number" className="mt-2 input-field" />
                  </div>
                </div>
                
                <div>
                  <Label htmlFor="visitDate">Date of Visit:</Label>
                  <Input id="visitDate" type="date" className="mt-2 input-field" />
                </div>
